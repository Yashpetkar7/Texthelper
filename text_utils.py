...
def count_substring(text, sub): return text.count(sub) if sub else 0
def get_preview_lines(text, n=20): return text.splitlines()[:n] if text else [""]
def get_stats(text):
    lines = text.splitlines() if text else []
    return len(lines), sum(len(l.split()) for l in lines), len(text or "")
